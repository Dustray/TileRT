...
| `lm_head.weight` | `lm_head.weight` | 沿 `vocab_size` 切分 | `(vocab_size, dim)` → `(num_devices, vocab_size/num_devices, dim)` |
...
> `lm_head` 是 Qwen3.6 中唯一按 `vocab_size` 维度做 TP 切分的权重。
...
| `RMSNormHeadProj` | `lm_head.weight` | `vocab_size` | TP8 | ❌（每张卡直接出部分 logits） |
...
| **Vocab TP8** | lm_head 词表维度 | 每个卡持有 `vocab_size/8` 的 head | all-gather logits（golden 未做，直接复制 full head） | ✅ |
