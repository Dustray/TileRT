...
class DeltaNetOp(TileRTModule):
    ...
    def _gated_delta_attention(
        ...
        """Reference gated DeltaNet recurrence matching the official chunk rule.

        This is a pure-PyTorch reimplementation of ``torch_chunk_gated_delta_rule``
        from ``transformers.models.qwen3_5_moe.modeling_qwen3_5_moe``.  It is used as the
        golden fallback so that TileRT's CPU/DCU reference path is bit-level compatible
        with the official model without relying on ``fla`` / ``causal-conv1d``.
        """
        ...
        # L2 normalization must be applied per-head, matching the official
        # ``torch_chunk_gated_delta_rule`` implementation.  Normalising on
        # the concatenated projection vector (head_dim * n_heads) yields a
        # ~sqrt(n_heads) smaller scale and produces gibberish outputs.
