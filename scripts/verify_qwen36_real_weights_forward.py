...
def main():
    ...
    logger.info("[2/3] Loading real pretrained weights...")
    ...
    logger.info("[2/3] Real weights loaded OK")

    logger.info("[3/5] Running forward(token_id=100, cur_pos=0)...")
    ...
    logger.info(
        "[3/5] next_token=%d, logits.shape=%s, finite=%s",
        next_token,
        tuple(logits.shape),
    ...
    logger.info("[4/5] Running three autoregressive decode steps...")
    for step, pos in enumerate([1, 2, 3], start=1):
        ...
        next_token = layer.get_next_token(device_id=0)
        logger.info("  step %d (cur_pos=%d) -> next_token=%d", step, pos, next_token)
        assert 0 <= next_token < model_args.vocab_size, f"Invalid next_token {next_token}"

    logger.info("[5/5] Checking logits per-device consistency...")
