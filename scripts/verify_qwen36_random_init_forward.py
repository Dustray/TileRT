...
def main():
    ...
    logger.info("[1/4] Importing Qwen3.6 modules...")
    from tilert.models.qwen3_6.model_args import ModelArgsQwen36
    from tilert.models.qwen3_6.modules.end2end import QwenShowHandsLayer
    logger.info("[1/4] Import OK")

    logger.info("[2/4] Building ModelArgsQwen36 (max_seq_len=512 for smoke test)...")
    model_args = ModelArgsQwen36()
    model_args.max_seq_len = 512
    model_args.max_batch_size = 1
    logger.info("[2/4] %s", model_args)

    logger.info("[3/4] Initializing QwenShowHandsLayer with random weights on 8 devices...")
    ...
    logger.info("[3/4] Random weights initialized OK")

    logger.info("[4/4] Running forward(token_id=100, cur_pos=0) on golden path...")
    ...
    logger.info(
        "[4/4] next_token=%d, logits.shape=%s, logits.finite=%s",
        next_token,
        tuple(logits.shape),
